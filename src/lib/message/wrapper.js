...
class AIRich extends BaseBuilder {
	...
	addHtml(html, { text = 'HTML content', trustedSources = [], id, replace, insertAt } = {}) {
		...
		if (!Array.isArray(trustedSources)) {
			throw new TypeError('trustedSources must be an array');
		}
...
class Foad extends BaseBuilder {
	#client;
	...
	setResponseId(id) {
		if (typeof id !== 'string') {
			throw new TypeError('Response id must be a string');
		}
	...
	setBotResponseId(id) {
		if (typeof id !== 'string') {
			throw new TypeError('Bot response id must be a string');
		}
	...
	addHtml(
		...
		if (typeof title !== 'string') {
			throw new TypeError('Tab title must be a string');
		}
		...
		if (!Array.isArray(stepEntries)) {
			throw new TypeError('stepEntries must be an array');
		}
		...
		if (this._tabs.some(tab => tab.id === tabId)) {
			throw new Error(`Tab id "${tabId}" already exists`);
		}
	 ...
	 * Add raw FOAD tab
	 *
	 * Useful when you want full control over
	 * sections / primitive structure.
	 */
	addTab(tab = {}) {
		if (
			!tab ||
			...
			throw new TypeError('Tab must be a plain object');
		}
		...
		if (typeof tab_header !== 'string') {
			throw new TypeError('Tab header must be a string');
		}
		...
		if (!Array.isArray(step_entries)) {
			throw new TypeError(
				'step_entries must be an array'
			);
		}

		if (this._tabs.some(item => item.id === id)) {
			throw new Error(
				`Tab id "${id}" already exists`
			);
		}
	...
	/**
	 * Build decoded FOAD response object
	 *
	 * Structure:
	 *
	 * {
	...
	buildResponse() {
		if (!this._tabs.length) {
			throw new Error(
				'Foad requires at least one HTML tab'
			);
		}
